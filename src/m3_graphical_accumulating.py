...
def draw_parallel_lines(n, point, length, window):
    ...
    for k in range(n):
        ...
        mho = rg.Line(startpoint, endpoint)
        mho.attach_to(window)
...
def draw_lines(n, point, window):
    ...
    for k in range(n):
        startpoint = point
        endpoint = rg.Point(startpoint.x + 100, startpoint.y - 100 + k * 200 / (n - 1))
        ohm = rg.Line(startpoint, endpoint)
        ohm.attach_to(window)
